...
const StudentsForRoundFour = () => {
  ...
  useEffect(() => {
    ...
    const filteredStudents = storedStudents.filter(student => storedRoundFourStudentIds.includes(student.srno));
  ...
  const handleDeleteStudent = (studentId) => {
    const updatedRoundFourStudents = roundFourStudents.filter(student => student.srno !== studentId);
    setRoundFourStudents(updatedRoundFourStudents);
    const updatedRoundFourStudentIds = updatedRoundFourStudents.map(student => student.srno);
    localStorage.setItem('roundFourStudentIds', JSON.stringify(updatedRoundFourStudentIds));
  };
...
//     ];
//     const filteredStudents = storedStudents.filter(student => storedRoundFourStudentIds.includes(student.srno));
//     setRoundFourStudentIds(filteredStudents);
...
//   const handleDeleteStudent = (studentId) => {
//     const updatedRoundFourStudents = roundFourStudentIds.filter(student => student.srno !== studentId);
//     setRoundFourStudentIds(updatedRoundFourStudents);
//     const updatedRoundFourStudentIds = updatedRoundFourStudents.map(student => student.srno);
//     localStorage.setItem('roundFourStudentIds', JSON.stringify(updatedRoundFourStudentIds));
//   };
...
//           </thead>
//           <tbody className="text-gray-600 text-sm font-light">
//             {roundFourStudentIds.map((student) => (
//               <tr key={student.srno}>
//                 <td className="py-3 px-6 text-left whitespace-nowrap">{student.srno}</td>
//                 <td className="py-3 px-6 text-left whitespace-nowrap">{student.tpoid}</td>
//                 <td className="py-3 px-6 text-left whitespace-nowrap">{student.name}</td>
